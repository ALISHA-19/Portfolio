...
const metricNotes = {
  "Lifecycle Emails Shipped": "60+ personalized, audience-adapted emails across 5 customer journey stages — generated from just 3 prompt documents.",
  "Conversion Lift": "Owned the acquisition-to-retention funnel; segmentation and journey mapping drove a 22% conversion lift across the portfolio.",
  "Global Products Owned": "End-to-end growth-funnel ownership across S&P Global's 150+ global B2B and B2C digital products.",
  "Years in Product": "Product growth, operations, and AI product management across DivineAPI, S&P Global, WNS, and Tech Mahindra.",
};
...
const tagTone = (tag) => {
  ...
  // CORE (Hard Skills / Techniques / Methodologies / Growth) -> Emerald
  if (value.includes('modeling') || value.includes('risk') || value.includes('data engineering') || value.includes('dashboarding') ||
      ...
      value.includes('funnel') || value.includes('segmentation') || value.includes('lifecycle') || value.includes('retention') ||
      value.includes('conversion') || value.includes('experiment') || value.includes('metric') || value.includes('evaluation') ||
      value.includes('knowledge base') || value.includes('attribution') || value.includes('kpi') || value.includes('a/b') ||
      value.includes('automation') || value.includes('hygiene') || value.includes('data analysis'))
    return 'tone-emerald';
  ...
  if (value.includes('strategy') || value.includes('vision') || value.includes('product') || value.includes('fintech') ||
      ...
      value.includes('communication') || value.includes('presentation') || value.includes('storytelling') || value.includes('roadmap') ||
      value.includes('discovery') || value.includes('gtm') || value.includes('documentation') || value.includes('alignment') ||
      value.includes('partnership') || value.includes('coordination') || value.includes('ownership') || value.includes('agentic') || value.includes('agile'))
    return 'tone-violet';
...
const skillNotes = {
  "AI Product Execution": "Shipping agentic AI end-to-end—RAG architecture, prompt systems, evaluation, and human-in-the-loop design, written down in PRDs and specs.",
  "Growth & Lifecycle": "Moving the funnel with evidence—segmentation, journey mapping, A/B testing, and North Star metrics across acquisition to retention.",
  "Data & Decision Making": "Turning data into decisions—dashboards, product analytics, and honest metrics that leadership can actually act on.",
  "Tools & Cross-functional": "The operating layer—Marketo, Salesforce, and agile delivery, leading engineers and designers across global B2B and B2C products.",
};
        ...
        >
          ...
          <div className="relative z-10 h-full flex flex-col">
            ...
            <h3 className="mt-8 text-2xl md:text-3xl font-semibold text-white tracking-normal leading-tight flex items-center gap-3">
              <span className="w-[1.5px] h-8 bg-[#38BDF8]/40" /> {category}
            </h3>
...
/* ── Research / IP Showcase removed — not applicable to this profile. ── */
    ...
    <div className="space-y-12">
      {education.map((edu) => (
        <motion.div
          ...
          className="research-card cursor-default border-white/10 relative overflow-hidden group transition-all duration-500 hover:border-[#38BDF8]/40"
          initial="rest"
          whileInView="active"
          whileHover="active"
        ...
        >
          ...
          <div className="grid lg:grid-cols-[0.4fr_0.6fr] gap-20 relative z-10 items-center">
            ...
            <div className="relative">
              <div className="flex flex-col gap-4 mb-10">
                <div className="flex items-center gap-3">
                  ...
                  {edu.duration && (
                    <span className="text-[10px] font-mono text-stone-500 uppercase tracking-widest">{edu.duration}</span>
                  )}
                </div>
                <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-emerald-500/60 font-bold group-hover:text-emerald-500 transition-colors duration-500">{edu.kicker || 'ACADEMIC FOUNDATION'}</p>
              </div>
            ...
            {/* Right Column: Capstone & Coursework */}
            <div className="space-y-12 border-l border-white/5 pl-12">
              {edu.highlight && (
                <div className="p-8 rounded-[2rem] bg-black/60 border border-emerald-500/10 backdrop-blur-2xl relative group-hover:border-[#38BDF8]/30 transition-colors duration-500">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-400 mb-5 flex items-center gap-2">
                    {edu.highlightLabel || 'CAPSTONE'}
                  </p>
                  <p className="text-xl text-stone-100 font-medium leading-relaxed italic mb-6">
                    "{edu.highlight}"
                  </p>
                  {edu.highlightTags && edu.highlightTags.length > 0 && (
                    <div className="flex flex-wrap items-center gap-3 text-[9px] uppercase tracking-widest text-emerald-500/60 font-mono">
                      {edu.highlightTags.map((tag) => (
                        <span key={tag} className="px-3 py-1.5 bg-emerald-500/5 border border-emerald-500/10 rounded-lg">{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
          ...
          <motion.div
            className="research-card-line bg-[#38BDF8]"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
        {certifications.map((cert) => {
          ...
          const theme = brandThemes[cert.vendor] || { color: '#38BDF8', pill: 'tone-gold', glow: 'rgba(56, 189, 248, 0.1)' };
          ...
          return (
            <motion.div
              ...
              className="research-card flex flex-col cursor-default group transition-all duration-500 hover:border-[#38BDF8]/40"
              style={{ 
                background: 'rgba(255, 255, 255, 0.08)',
            ...
            >
              ...
              <div className="relative z-10 flex-1 flex flex-col">
                ...
                {cert.id && (
                  <p className="font-mono text-[10px] text-[#38BDF8] uppercase tracking-widest mb-6 opacity-60">
                    Cred ID: {cert.id}
                  </p>
                )}
                ...
                <div className="flex-1">
                  <div className="mb-8">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[#38BDF8] mb-3 opacity-80">Strategic Rationale</p>
                  ...
                  <div className="mb-6">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-3 group-hover:text-[#38BDF8]/80 transition-colors flex items-center gap-2">
    ...
    <div id="home" className="overflow-hidden">
      <section className="relative min-h-screen px-6 pt-28 pb-20 flex items-center hero-section">
        <div className="absolute inset-0 z-0">
          {loadScene ? (
            <Suspense fallback={<div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_35%,rgba(56, 189, 248,0.12),transparent_28rem)]" />}>
              <ThreeHeroScene />
            </Suspense>
          ) : (
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_35%,rgba(56, 189, 248,0.12),transparent_28rem)]" />
          )}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_65%_35%,rgba(56, 189, 248,0.16),transparent_28%),linear-gradient(90deg,#0A0F1A_0%,rgba(11,11,15,0.92)_38%,rgba(11,11,15,0.52)_100%)]" />
        </div>
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto w-full grid lg:grid-cols-[1.05fr_0.95fr] gap-12 items-center">
          <motion.div initial="hidden" animate="show">
            <motion.p variants={fadeUp} className="section-kicker mb-6">
              AI products · Growth & lifecycle · B2B / API
            </motion.p>
            <motion.h1 variants={fadeUp} custom={0.08} className="hero-heading max-w-5xl">
              I build AI products people can trust.
            </motion.h1>
            <motion.p variants={fadeUp} custom={0.16} className="mt-8 max-w-xl text-lg text-stone-300 leading-relaxed">
              Alisha Chaudhari is an AI Product Manager who ships agentic AI end-to-end — grounded in RAG, verified before it reaches a customer, and built on 4+ years of growth and lifecycle product work.
            </motion.p>
            <motion.div variants={fadeUp} custom={0.24} className="mt-10 flex flex-wrap gap-4">
              ...
              <a href="/Alisha_Chaudhari_CV.pdf" download className="btn-secondary">Download CV</a>
            </motion.div>
          </motion.div>
          ...
          >
            <div className="hero-manifesto">
              ...
              <p className="text-3xl font-semibold tracking-normal text-white leading-tight">
                Make AI trustworthy, make growth measurable, and make the product impossible to ignore.
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <section id="projects" className="section-shell">
        <SectionHeader
          ...
          title="Real products, real outcomes — not template case studies."
          copy="The work spans agentic AI and RAG systems, growth funnels, marketing automation, and lifecycle experimentation."
        />
        <ProjectWorkbench
      ...
      <section id="journey" className="section-shell">
        <SectionHeader
          ...
          title="The path from product operations to AI product leadership."
          copy="Each role added a layer — customer experience, product operations, growth and lifecycle, and now end-to-end AI product delivery."
        />
      ...
      <section id="skills" className="section-shell">
        <SectionHeader
          kicker="Skills"
          title="A product toolkit with real AI depth."
          copy="Built around four areas: AI product execution, growth and lifecycle, data and decisions, and cross-functional delivery."
        />
        <SkillsConstellation />
      </section>
      ...
      <section id="credentials" className="section-shell py-24">
        <SectionHeader
          kicker="Credentials"
          title="Foundation, training, and recognition."
          copy="AI product management training, professional certifications, and the academic foundation behind the work."
        />
        <CredentialsShowcase />
      </section>
      ...
      <section id="contact" className="section-shell pb-28">
        ...
        >
          <div>
            ...
            <h2 className="editorial-heading max-w-3xl">Have a product worth shipping?</h2>
            <p className="mt-6 max-w-xl text-stone-400 leading-relaxed">
              Open to AI Product Manager and Product Manager roles, and to building agentic AI products with teams that care about trust and growth.
            </p>
          ...
          <div className="grid gap-3">
            ...
            <a href={portfolioData.linkedin} target="_blank" rel="noreferrer" className="contact-link accent-linkedin">
              <LinkedInIcon className="h-5 w-5" />
              <span>LinkedIn</span>
            </a>
            <a href={portfolioData.github} target="_blank" rel="noreferrer" className="contact-link accent-github">
              <GitHubIcon className="h-5 w-5" />
              <span>GitHub</span>
            </a>
            <a href={portfolioData.portfolio} target="_blank" rel="noreferrer" className="contact-link">
              <span>Portfolio ↗</span>
            </a>
            <a href={`https://wa.me/${portfolioData.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noreferrer" className="whatsapp-button mt-3">
